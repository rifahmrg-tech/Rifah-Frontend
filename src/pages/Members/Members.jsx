...
const Members = () => {
  const { user } = useAuth(); // To show Admin-only controls if needed
  const [members, setMembers] = useState([]);
  ...
  // Filter Logic (Search by Name, Company, or Area)
  const filteredMembers = members.filter(member => 
    member.name?.toLowerCase().includes(filter.toLowerCase()) ||
    member.companyName?.toLowerCase().includes(filter.toLowerCase()) ||
    member.address?.city_area?.toLowerCase().includes(filter.toLowerCase())
  );
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <div>
          ...
          <p>Verified businesses in the Trichy Zone</p>
        </div>
            ...
            <tbody>
              {filteredMembers.map((member) => (
                <tr key={member._id}>
                  ...
                  <td className={styles.companyName}>{member.companyName || "N/A"}</td>
                  <td>{member.name}</td>
                  <td>
                    <div className={styles.areaTag}>
                      <MapPin size={14} /> 
                      {member.address?.city_area || "Trichy City"}
                    </div>
                  </td>
                  <td>{member.businessCategory || "General"}</td>
                  <td>
                    {member.phone && (
                      <a 
